...
const payment = () => {
  ...
  let result = 0;
  const foodRequest = restaurant.fetchMenu().food;
  const drinkRequest = restaurant.fetchMenu().drink;

  for (let i = 0; i < consum.length; i += 1) {
    if (Object.keys(foodRequest).includes(consum[i])) {
      result += foodRequest[consum[i]];
    } else if (Object.keys(drinkRequest).includes(consum[i])) {
      result += drinkRequest[consum[i]];
    }
  }
  return result * 1.1;
};
...
// TESTE DE FUNCIONAMENTO ABAIXO:

/* const meuRestaurante = createMenu({ food: { coxinha: 13.9, sopa: 19.9 }, drink: { agua: 3.9, cerveja: 6.9 } });
 */
/* console.log(meuRestaurante.fetchMenu()); */
// Retorno: { food: {'coxinha': 13.9, 'sopa': 19.9}, drink: {'agua': 3.9, 'cerveja': 6.9} }
...
// Retorno: [ 'coxinha', 'agua', 'sopa', 'cerveja' ]

/* console.log(meuRestaurante.pay());
 */// Retorno: 49.06

// console.log(restaurant.fetchMenu().food['coxinha']); => isso mostra o valor da chave
// console.log(foodRequest[consum[0]]); 
